...
df: DataFrame = pandas.read_excel(
    io="Author_Label_Mapping.xlsx", header=None, names=["key", "author"]
)
...
mmnb: MultinomialNB = load(filename="../models/MultinomialNB_TFIDF.joblib")
tfidf: TfidfVectorizer = load(filename="../models/TfidfVectorizer.joblib")

vector: spmatrix = tfidf.transform(raw_documents=passage)
prediction: ndarray = mmnb.predict(X=vector)

predictionInt: int = prediction[0]
predictionAuthor: str = df["author"].iloc[prediction[0] - 1]

print("Prediction: ", predictionAuthor, predictionInt)

print("Correct Author:  James Baldwin 8")
